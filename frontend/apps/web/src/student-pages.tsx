...
export function StudentProfile() {
  ...
  return <><StudentPageHeader eyebrow="Your account" title="Profile" copy="Your personal learning profile from CODEAN." /><section className="profile-grid"><article className="profile-card"><span className="profile-avatar">{profile.fullName.split(" ").map((part) => part[0]).join("").slice(0, 2).toUpperCase()}</span><h2>{profile.fullName}</h2><p>{profile.email}</p><span className="status-badge success">Student</span></article><section className="table-panel"><h2>Profile details</h2><dl className="detail-list"><div><dt>Email</dt><dd>{profile.email}</dd></div><div><dt>Phone</dt><dd>{profile.phone || "Not provided"}</dd></div><div><dt>Grade</dt><dd>{profile.grade || "Not provided"}</dd></div><div><dt>School</dt><dd>{profile.school || "Not provided"}</dd></div></dl><Link className="button button-primary" to="/settings">Edit profile</Link></section></section></>;
}
...
export function StudentSettings() {
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); try { const result = await usersApi.updateStudentProfile({ fullName: String(data.get("fullName")), phone: String(data.get("phone") || ""), grade: String(data.get("grade") || ""), school: String(data.get("school") || "") }); setProfile(result); setSaved(true); } catch (cause) { setError(messageFor(cause, "Unable to save settings.")); } };
  if (loading) return <LoadingState />; if (!profile) return <ErrorState message={error || "Settings are unavailable."} />;
  return <><StudentPageHeader eyebrow="Account settings" title="Settings" copy="Keep your CODEAN profile information current." />{error && <p className="form-error">{error}</p>}<form className="editor-form" onSubmit={save}><section><label className="form-field"><span>Full name</span><input name="fullName" defaultValue={profile.fullName} required /></label><label className="form-field"><span>Phone</span><input name="phone" defaultValue={profile.phone} /></label><div className="form-grid two"><label className="form-field"><span>Grade</span><input name="grade" defaultValue={profile.grade} /></label><label className="form-field"><span>School</span><input name="school" defaultValue={profile.school} /></label></div></section><aside><p className="eyebrow">Privacy</p><p className="muted-copy">Authentication is managed by Clerk. Profile data is stored by the CODEAN backend in Supabase.</p><button className="button button-primary">Save changes</button>{saved && <span className="saved-message"><Check size={14} /> Changes saved</span>}</aside></form></>;
}
