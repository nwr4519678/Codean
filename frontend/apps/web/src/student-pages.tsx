...
const helpArticles = [
  { title: "How do I enroll in a course?", body: "Open the course catalog, choose a published course, and select Enroll for free or Continue to secure checkout. Paid access is activated only after the payment provider confirms your payment." },
  { title: "Where can I see my course progress?", body: "Open My learning or Overview from the sidebar. Your enrolled courses, lesson count, completion percentage, and access type are loaded from your CODEAN account." },
  { title: "How do I update my profile?", body: "Open Settings, edit your name, phone, grade, or school, then select Save changes. Your profile is stored through the CODEAN API in Supabase." },
  { title: "How do I change my profile photo?", body: "Select your profile avatar or Change photo. Clerk manages profile photos and automatically shows photos from supported sign-in providers." },
  { title: "I cannot sign in or verify my session.", body: "Refresh the page once, then use Sign out and sign in again. If the problem continues, contact support and include the exact message shown on screen without sharing your password." },
];

export function StudentHelpCenter() {
  const [query, setQuery] = useState("");
  const [open, setOpen] = useState<number | null>(0);
  const filtered = helpArticles.filter((article) => `${article.title} ${article.body}`.toLowerCase().includes(query.toLowerCase()));

  return <><StudentPageHeader eyebrow="Support" title="Help center" copy="Find clear answers about your account, courses, progress, and security." action={<a className="button button-primary" href="mailto:support@codean.app"><Mail size={15} /> Contact support</a>} /><section className="help-center-layout"><article className="help-welcome"><span className="help-welcome-icon"><CircleHelp size={25} /></span><div><p className="eyebrow">CODEAN support</p><h2>How can we help?</h2><p>Search the most common questions or contact the support team when you need more help.</p></div><label className="help-search"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search help articles" aria-label="Search help articles" /></label></article><section className="help-faq-card"><div className="section-heading compact"><div><p className="eyebrow">Frequently asked questions</p><h2>Answers for your workspace</h2></div><span className="help-article-count">{filtered.length} articles</span></div>{filtered.length ? <div className="help-faq-list">{filtered.map((article) => { const index = helpArticles.indexOf(article); const isOpen = open === index; return <article className={isOpen ? "help-faq-item open" : "help-faq-item"} key={article.title}><button type="button" aria-expanded={isOpen} onClick={() => setOpen(isOpen ? null : index)}><span>{article.title}</span><ChevronRight size={18} /></button>{isOpen && <p>{article.body}</p>}</article>; })}</div> : <EmptyState title="No matching articles" copy="Try a different search phrase or contact support." />}</section><aside className="help-contact-card"><span><MessageSquareText size={18} /></span><div><h2>Still need help?</h2><p>Our support team can help with account and access issues.</p></div><a className="button button-outline" href="mailto:support@codean.app">Email support</a></aside></section></>;
}
