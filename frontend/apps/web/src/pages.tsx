...
export function AuthPage() {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); if (mode === "login") navigate(role === "Teacher" ? "/teacher/dashboard" : "/dashboard"); else setSubmitted(true); };
  ...
  return <div className="auth-page"><aside><Link to="/" className="brand"><span className="brand-mark"><Code2 size={20} /></span>CODEAN</Link><div><p className="eyebrow">Focused learning</p><h2>Small steps. Real projects. Visible progress.</h2><p>Join a workspace built around doing the work, reviewing feedback, and improving deliberately.</p></div><span>Trusted by 18,000+ learners</span></aside><main><div className="auth-card"><Link className="back-link" to="/"><ArrowLeft size={15} /> Back to CODEAN</Link><p className="eyebrow">Account access</p><h1>{title}</h1><p>{copy}</p><form onSubmit={submit}>{mode === "register" && <Field label="Full name"><input required placeholder="Nadia Hassan" /></Field>}{mode === "login" && <Field label="Workspace role"><select value={role} onChange={(e) => setRole(e.target.value)}><option>Student</option><option>Teacher</option></select></Field>}{!["2fa", "verify-email"].includes(mode) && <Field label="Email address"><input required type="email" placeholder="name@example.com" /></Field>}{["login", "register"].includes(mode) && <Field label="Password"><input required type="password" placeholder="••••••••••••" /></Field>}{mode === "reset-password" && <><Field label="New password"><input required type="password" /></Field><Field label="Confirm password"><input required type="password" /></Field></>}{["2fa", "verify-email"].includes(mode) && <Field label="Verification code"><input className="code-input" required inputMode="numeric" maxLength={6} placeholder="000000" /></Field>}<button className="button button-primary auth-submit">{mode === "login" ? "Sign in" : mode === "register" ? "Create student account" : "Continue"}<ArrowRight size={16} /></button></form>{mode === "login" && <><Link className="auth-secondary" to="/auth/forgot-password">Forgot password?</Link><p className="auth-switch">New to CODEAN? <Link to="/auth/register">Create a student account</Link></p></>}{mode === "register" && <><p className="registration-note"><ShieldCheck size={14} /> Teacher accounts are created by a platform administrator.</p><p className="auth-switch">Already have an account? <Link to="/auth/login">Sign in</Link></p></>}</div></main></div>;
}
...
export function TeacherCourses() {
  return <><PageHeader eyebrow="Course management" title="Courses" copy="Create, publish, and maintain your learning catalog." action={<Link className="button button-primary" to="/teacher/courses/new"><Plus size={16} /> Create course</Link>} /><section className="table-panel"><table><thead><tr><th>Course</th><th>Category</th><th>Students</th><th>Price</th><th>Status</th><th /></tr></thead><tbody>{courses.map((course, index) => <tr key={course.id}><td><Link to={`/teacher/courses/${course.id}`}><strong>{course.title}</strong><small>{course.lessons} lessons</small></Link></td><td>{course.category}</td><td>{index === 3 ? 0 : 246 + index * 91}</td><td>EGP {index === 1 ? "Free" : "1,250"}</td><td><span className={`status-badge ${index === 3 ? "warning" : "success"}`}>{index === 3 ? "Draft" : "Published"}</span></td><td><Link className="icon-button" to={`/teacher/courses/${course.id}/edit`}><Pencil size={15} /></Link></td></tr>)}</tbody></table></section></>;
}

export function TeacherCourseOverview() {
  const { id } = useParams();
  const course = courses.find((item) => item.id === id) ?? courses[0];
  return <><PageHeader eyebrow="Course management" title={course.title} copy="Review curriculum, publishing state, and learner activity." action={<div className="button-row"><Link className="button button-outline" to={`/teacher/courses/${course.id}/edit`}><Pencil size={15} /> Edit</Link><Link className="button button-primary" to={`/teacher/courses/${course.id}/modules`}>Manage curriculum</Link></div>} /><div className="metric-row"><Metric icon={Users} value="428" label="Students" /><Metric icon={BookOpen} value={`${course.lessons}`} label="Lessons" tone="green" /><Metric icon={Clock3} value={course.duration} label="Runtime" tone="amber" /><Metric icon={TrendingUp} value="73%" label="Completion" tone="coral" /></div><section className="curriculum-overview"><div className="section-heading"><div><p className="eyebrow">Course structure</p><h2>Curriculum</h2></div></div>{curriculum.map((module, index) => <article key={module.title}><span>{index + 1}</span><div><h3>{module.title}</h3><p>{module.lessons.join(" · ")}</p></div><strong>{module.lessons.length} lessons</strong></article>)}</section></>;
}
...
export function TeacherCourseEditor() {
  ...
  return <><PageHeader eyebrow="Course editor" title={course ? "Edit course" : "Create a course"} copy="Define the course offer before building its modules and lessons." /><form className="editor-form" onSubmit={(e) => { e.preventDefault(); navigate(course ? `/teacher/courses/${course.id}` : "/teacher/courses/react/modules"); }}><section><Field label="Course title"><input required defaultValue={course?.title} placeholder="Production React Architecture" /></Field><Field label="Description"><textarea required defaultValue={course ? "A practical course for building maintainable frontend systems." : ""} /></Field><div className="form-grid two"><Field label="Category"><select defaultValue={course?.category ?? "Frontend"}><option>Frontend</option><option>Backend</option><option>Programming</option><option>Computer Science</option></select></Field><Field label="Price (EGP)"><input type="number" defaultValue={course ? 1250 : 0} /></Field></div><Field label="Thumbnail URL"><input type="url" placeholder="https://" /></Field></section><aside><p className="eyebrow">Publishing checklist</p>{["Clear course outcome", "Complete description", "Category selected", "Pricing confirmed"].map((item) => <p key={item}><CheckCircle2 size={15} /> {item}</p>)}<button className="button button-primary">{course ? "Save changes" : "Create and add modules"}</button></aside></form></>;
}

export function TeacherModules() {
  const [modules, setModules] = useState(curriculum.map((item) => ({ ...item })));
  const [title, setTitle] = useState("");
  return <><PageHeader eyebrow="Curriculum builder" title="Course modules" copy="Organize lessons into a clear learning sequence." action={<button className="button button-primary" onClick={() => { if (title.trim()) setModules([...modules, { title: title.trim(), lessons: [] }]); setTitle(""); }}><Plus size={16} /> Add module</button>} /><div className="module-builder-toolbar"><input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="New module title" /></div><div className="module-builder">{modules.map((module, moduleIndex) => <article key={`${module.title}-${moduleIndex}`}><header><GripVertical size={17} /><span>{moduleIndex + 1}</span><div><h2>{module.title}</h2><p>{module.lessons.length} lessons</p></div><button className="icon-button" onClick={() => setModules(modules.filter((_, index) => index !== moduleIndex))}><Trash2 size={16} /></button></header>{module.lessons.map((lesson, lessonIndex) => <div className="builder-lesson" key={lesson}><Play size={14} /><span>{lesson}</span><small>{18 + lessonIndex * 4} min</small><button className="icon-button"><Pencil size={14} /></button></div>)}<button className="add-lesson" onClick={() => setModules(modules.map((item, index) => index === moduleIndex ? { ...item, lessons: [...item.lessons, `New lesson ${item.lessons.length + 1}`] } : item))}><Plus size={14} /> Add lesson</button></article>)}</div></>;
}
